...
class Solution:
    ...
    def _dfs(self, node: int, parent: int, visited: Set[int], lowest: List[int], index: List[int], curr_level: int, graph: List[List[int]], res: List[List[int]]):
        visited.add(node)
        ...
        for nei in graph[node]:
            ...
            if nei not in visited:
                self._dfs(nei, node, visited, lowest, index, curr_level + 1, graph, res)     # if haven't visited this neighbor, add it into this path
            lowest[node] = min(lowest[node], lowest[nei])   # after the neighbor is analyzed, we can know that it we can find a shorter path
